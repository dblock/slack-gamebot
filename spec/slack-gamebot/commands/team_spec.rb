...
describe SlackGamebot::Commands::Team, vcr: { cassette_name: 'user_info' } do
  ...
  context 'with a captain' do
    ...
    it 'team' do
      expect(message: "#{SlackRubyBot.config.user} team").to respond_with_slack_message "Team _#{team.name}_, captain username."
    end
  end
  context 'with two captains' do
    ...
    it 'team' do
      expect(message: "#{SlackRubyBot.config.user} team").to respond_with_slack_message "Team _#{team.name}_, captains #{team.captains.map(&:user_name).and}."
    end
  end
end
