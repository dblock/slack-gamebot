...
describe SlackGamebot::Commands::Register, vcr: { cassette_name: 'user_info' } do
  ...
  it 'registers a new user and promotes them to captain' do
    ...
    expect do
      expect(message: "#{SlackRubyBot.config.user} register", user: 'user').to respond_with_slack_message("Welcome <@user>! You're ready to play. You're also a team captain.")
  ...
  it 'renames an existing user' do
    ...
    expect do
      expect(message: "#{SlackRubyBot.config.user} register", user: 'user').to respond_with_slack_message("Welcome back <@user>, I've updated your registration. You're also a team captain.")
    end.to_not change(User, :count)
  end
  it 'already registered' do
    Fabricate(:user, user_id: 'user', user_name: 'username', captain: true)
    expect do
      expect(message: "#{SlackRubyBot.config.user} register", user: 'user').to respond_with_slack_message("Welcome back <@user>, you're already registered. You're also a team captain.")
    end.to_not change(User, :count)
  end
end
