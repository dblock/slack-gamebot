...
describe SlackGamebot::Commands::Challenge, vcr: { cassette_name: 'user_info' } do
  ...
  it 'requires an opponent' do
    expect do
      expect(message: 'gamebot challenge', user: user.user_id, channel: 'pongbot').to respond_with_error(
  ...
  it 'does not butcher names with special characters' do
    expect(message: 'gamebot challenge Jung-hwa', user: user.user_id, channel: 'pongbot').to respond_with_error(
