shared_examples_for 'a cursor api' do |model|
  ...
  context model.name do
    ...
    it 'returns 10 items by default' do
      expect(client.send(model_ps, {}).count).to eq 10
    end
    ...
    it 'returns a first page with a cursor' do
      response = client.send(model_ps, size: 2)
      expect(response._links.self._url).to eq "http://example.org/#{model_ps}?size=2"
      expect(response._links.next._url).to start_with "http://example.org/#{model_ps}?"
    ...
    it 'paginates over the entire collection' do
      ...
      next_cursor = { size: 3 }
      loop do
        response = client.send(model_ps, next_cursor)
        models_ids.concat(response.map { |instance| instance._links.self._url.gsub("http://example.org/#{model_ps}/", '') })
