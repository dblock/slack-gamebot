    ...
    class Demote < SlackRubyBot::Commands::Base
      def self.call(client, data, match)
        ...
        if !match.names.include?('expression') || match['expression'] != 'me'
          ...
          logger.info "DEMOTE: #{client.team.name} - #{user.user_name}, failed, not me"
        elsif !user.captain?
          send_message_with_gif client, data.channel, "You're not a captain, sorry.", 'sorry'
          logger.info "DEMOTE: #{client.team.name} - #{user.user_name}, failed, not captain"
          ...
          logger.info "DEMOTE: #{client.team.name} - #{user.user_name}, failed, last captain"
        else
          user.demote!
          send_message client, data.channel, "#{user.user_name} is no longer captain."
          logger.info "DEMOTED: #{client.team.name} - #{user.user_name}"
        end
      end
    end
  end
