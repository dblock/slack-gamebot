    ...
    class Promote < SlackRubyBot::Commands::Base
      def self.call(client, data, match)
        ...
        if !users
          ...
          logger.info "PROMOTE: #{user.user_name} promoting #{users.map(&:user_name).and}, failed, not captain"
        elsif captains && captains.count > 1
          send_message client, data.channel, "#{captains.map(&:user_name).and} are already captains."
          logger.info "PROMOTE: #{user.user_name} promoting #{users.map(&:user_name).and}, failed, #{captains.map(&:user_name).and} already captains"
        elsif captains && captains.count == 1
          send_message client, data.channel, "#{captains.first.user_name} is already a captain."
          logger.info "PROMOTE: #{user.user_name} promoting #{users.map(&:user_name).and}, failed, #{captains.first.user_name} already captain"
        else
          users.each(&:promote!)
